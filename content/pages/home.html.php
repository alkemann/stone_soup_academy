
<h2>Welcome</h2>
<p><b>Our goal</b>: help you improve your crawler skills from beginner to intermediate with the objective of competing in the CosPlay weekly challenge.</p>

<p><b>Our approach</b>: play one of five beginner level combos, each based on a Crawl Unique.</p> 
<p>              You'll be earning points by following specific conducts that help you develop good habits.</p>

<h3>Select your character guide:</h3>

<ul class="charlist">
    <li><img src="./img/Snorg.png"> <a href="#">Snorg, the troll berzerker of Trog</a></li>
    <li><img src="./img/Asterion.png"> <a href="#">Asterion, the minotaur fighter of Makhleb</a></li>
    <li><img src="./img/Nessos.png"> <a href="#">Nessos, the centaur hunter of Okawaru</a></li>
    <li><img src="./img/Saint_Roka.png"> <a href="#">Saint Roka, the Hill Orc Monk of Beogh</a></li>
    <li><img src="./img/Grum.png"> <a href="#">Grum, the gnoll monk of Yredelemnul</a></li>
</ul>

For tactical guides, see:
    <li><a href="http://www.ultraviolent4.com/guide.html#General_Tactics">UV4's General Tactics</a></li>
    <li><a href="http://www.ultraviolent4.com/patashutactics.html">Patashu's Crawl Tactics Guide</a></li>
